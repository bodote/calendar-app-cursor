...
class WoodleViewMvcTest
                extends ScenarioTest<GivenWoodleViewMvcStage, WhenWoodleViewMvcAction, ThenWoodleViewMvcOutcome> {
        ...
        @ScenarioStage
        private GivenWoodleViewMvcStage givenWoodleViewMvcState;
        @ScenarioStage
        ...
        void should_display_summary_and_store_in_s3() throws Exception {
                given().mock_mvc_is_configured(mockMvc)
                                ...
                                .and().user_is_on_homepage()
                                .and()
                                .user_has_test_data("John Doe", "john@example.com", "Team Meeting", "Weekly sync");
                when().user_clicks_schedule_event_button()
                                .and().user_sets_input_fields_on_schedule_event_page_and_clicks_next()
        ...
        void should_add_new_empty_row_after_saving_participant() throws Exception {
                ...
                then().events_table_should_have_exactly_two_participant_rows()
                                .and()
                                .events_table_should_show_fixed_participant_row("Alice", Map.of(0, true, 1, false))
                                .and().events_table_should_have_new_empty_participant_row();
        }
        ...
        @Test
        @DisplayName("Should be able to delete a participant from the proposed event times table")
        void should_be_able_to_delete_participant_from_events_table() throws Exception {
                String testUuid = "123e4567-e89b-12d3-a456-426614174005";

                given().mock_mvc_is_configured(mockMvc)
                                .and().object_mapper_is_configured(objectMapper)
                                .and().s3_client_is_configured(s3Client)
                                .and().s3_client_returns_three_test_events_for_uuid(testUuid)
                                .and().s3_client_returns_poll_data_for_uuid(testUuid,
                                                createPollDataWithParticipants(testUuid));

                when().user_deletes_participant("Alice", testUuid);

                then().participant_deletion_should_be_saved_to_poll()
                                .and().events_table_should_not_contain_participant("Alice")
                                .and().events_table_should_show_fixed_participant_row("Bob", Map.of(
                                                0, false,
                                                1, true,
                                                2, false))
                                .and().events_table_should_have_new_empty_participant_row()
        ...
        @Test
        @DisplayName("Should display delete buttons in the last column of each participant row")
        void should_display_delete_buttons_for_each_participant_row() throws Exception {
                String testUuid = "123e4567-e89b-12d3-a456-426614174006";
                ...
                then().events_table_should_have_delete_buttons_for_participants()
                                .and().events_table_should_show_multiple_fixed_participant_rows(Map.of(
                                                "Alice", Map.of(0, true, 1, false, 2, true),
                                                "Bob", Map.of(0, false, 1, true, 2, false)))
                                .and().events_table_should_have_new_empty_participant_row();
        ...
        // Add a helper method to create poll data with participants
        private PollData createPollDataWithParticipants(String uuid) throws Exception {
                List<PollData.EventTimeSlot> timeSlots = List.of(
                                new PollData.EventTimeSlot(LocalDate.parse("2024-03-15"), LocalTime.parse("09:00"),
                                                LocalTime.parse("10:00")),
                                new PollData.EventTimeSlot(LocalDate.parse("2024-03-15"), LocalTime.parse("14:00"),
                                                LocalTime.parse("15:00")),
                                new PollData.EventTimeSlot(LocalDate.parse("2024-03-18"), LocalTime.parse("11:00"),
                                                LocalTime.parse("12:00")));
                List<PollData.Participant> participants = List.of(
                                new PollData.Participant("Alice", List.of(0, 2)),
                                new PollData.Participant("Bob", List.of(1)));
                return new PollData(
                                JANE_SMITH_NAME,
                                JANE_SMITH_EMAIL,
                                PROJECT_PLANNING_TITLE,
                                QUARTERLY_PLANNING_DESCRIPTION,
                                timeSlots,
...
class GivenWoodleViewMvcStage extends Stage<GivenWoodleViewMvcStage> {
        @ScenarioState
        private MockMvc mockMvc;
        @ScenarioState
        private MockHttpSession session;
        ...
        public GivenWoodleViewMvcStage mock_mvc_is_configured(MockMvc mockMvc) {
                this.mockMvc = mockMvc;
                return self();
        }

        public GivenWoodleViewMvcStage object_mapper_is_configured(ObjectMapper objectMapper) {
                this.objectMapper = objectMapper;
                return self();
        }

        public GivenWoodleViewMvcStage s3_client_is_configured(S3Client s3Client) {
                this.s3Client = s3Client;
                return self();
        }

        public GivenWoodleViewMvcStage user_is_on_homepage() {
                session = new MockHttpSession();
                return self();
        }

        public GivenWoodleViewMvcStage user_has_test_data(String name, String email, String title, String description) {
                this.name = name;
                this.email = email;
                this.title = title;
        ...
        public GivenWoodleViewMvcStage s3_client_returns_test_event_for_uuid(String uuid) throws Exception {
        ...
        public GivenWoodleViewMvcStage s3_client_returns_three_test_events_for_uuid(String uuid) throws Exception {
                return s3_client_returns_test_event_for_uuid_with_time_slots(uuid,
        ...
        public GivenWoodleViewMvcStage s3_client_returns_test_event_for_uuid_with_time_slots(
                        String uuid,
                        String name,
                        String email,
                        String title,
        ...
        public GivenWoodleViewMvcStage s3_client_returns_poll_data_for_uuid(String uuid, PollData pollData)
                        throws Exception {
...
class WhenWoodleViewMvcAction extends Stage<WhenWoodleViewMvcAction> {
        ...
        private void addToHistory(String page) {
                ...
                if (currentPage != null) {
                        boolean isValidNavigation = switch (currentPage) {
                                ...
                                case "/schedule-event-step2" ->
                                        page.equals("/schedule-event-step3") || page.equals("/schedule-event");
        ...
        public WhenWoodleViewMvcAction user_sets_input_fields_on_schedule_event_page_and_clicks_next()
                        throws Exception {
        ...
        public WhenWoodleViewMvcAction user_sets_input_fields_on_schedule_event_step2_and_clicks_next(
                ...
                log.info("Setting input fields on schedule event step2 and submitting with {} time slots",
                                timeSlots.size());
        ...
        public WhenWoodleViewMvcAction user_sets_input_fields_on_schedule_event_step3_and_clicks_next()
                        throws Exception {
        ...
        public WhenWoodleViewMvcAction user_captures_event_url_and_reloads_in_new_session() throws Exception {
                ...
                ResponseInputStream<GetObjectResponse> responseStream = new ResponseInputStream<>(
                                ...
                                new ByteArrayInputStream(
                                                capturedBody.contentStreamProvider().newStream().readAllBytes()));
        ...
        public WhenWoodleViewMvcAction user_deletes_participant(String participantName, String eventUuid)
                        throws Exception {
                log.info("Deleting participant {} from event {}", participantName, eventUuid);

                // Perform the delete action
                resultAction = mockMvc.perform(delete("/event/" + eventUuid + "/participants/delete")
                                .param("participantName", participantName));

                // Verify that the delete action redirects to the correct event page
                resultAction.andExpect(status().isFound())
                                .andExpect(redirectedUrl("/event/" + eventUuid));
                ...
                // Follow the redirect to get the updated page content (like a real browser
                // would)
                log.info("Following redirect to /event/{} after deleting participant", eventUuid);
                resultAction = mockMvc.perform(get("/event/" + eventUuid));
                resultAction.andExpect(status().isOk());
...
class ThenWoodleViewMvcOutcome extends Stage<ThenWoodleViewMvcOutcome> {
        ...
        public ThenWoodleViewMvcOutcome summary_page_should_show_all_entered_data(
                ...
                assertThat(doc.select(
                                "div[data-test-section='poll-details'] div[data-test-item='title'] span:contains("
                                                + title + ")")
                                .size())
                                .isEqualTo(1);
                assertThat(doc.select(
                                "div[data-test-section='poll-details'] div[data-test-item='description'] span:contains("
                                                + description + ")")
                                .size())
                                .isEqualTo(1);
                ...
                assertThat(doc.select(
                                "div[data-test-section='organizer-details'] div[data-test-item='name'] span:contains("
                                                + name + ")")
                                .size())
                                .isEqualTo(1);
                assertThat(doc.select(
                                "div[data-test-section='organizer-details'] div[data-test-item='email'] span:contains("
                                                + email + ")")
                                .size())
                                .isEqualTo(1);
                ...
                for (TimeSlot slot : expectedTimeSlots.values()) {
                        Elements timeHeaders = doc
                                        .select("table[data-test='events-table'] thead tr th[data-test-date='"
                                                        + slot.date() + "']");
        ...
        public ThenWoodleViewMvcOutcome events_table_should_have_column_headers() throws Exception {
                ...
                assertThat(secondRowHeaders.size()).isEqualTo(3); // 3 time slot columns (no participant column as it's
                                                                  // rowspan)
        ...
        public ThenWoodleViewMvcOutcome events_table_should_group_dates_together() throws Exception {
                ...
                Elements dateHeaders = doc
                                .select("table[data-test='events-table'] thead tr:first-child th[data-test-date]");
        ...
        public ThenWoodleViewMvcOutcome events_table_should_show_fixed_participant_row(String participantName,
                ...
                Elements participantNameCells = doc
                                .select("table[data-test='events-table'] tbody tr td[data-test-participant='"
                                                + participantName + "']");
                ...
                for (Map.Entry<Integer, Boolean> entry : timeSlots.entrySet()) {
                        ...
                        Elements checkboxes = doc.select(
                                        "table[data-test='events-table'] tbody tr td:nth-child(" + (slotIndex + 2)
                ...
                for (Map.Entry<String, Map<Integer, Boolean>> participantEntry : participantsAndTimeSlots.entrySet()) {
                        ...
                        for (Map.Entry<Integer, Boolean> timeSlotEntry : timeSlots.entrySet()) {
                                ...
                                Elements checkboxes = participantRow
                                                .select("td:nth-child(" + (slotIndex + 2)
                                                                + ") input[type='checkbox'][disabled]");
                                assertThat(checkboxes.size()).isEqualTo(1);
        ...
        public ThenWoodleViewMvcOutcome participant_deletion_should_be_saved_to_poll() throws Exception {
                log.info("Verifying that participant deletion was saved to poll");
                verify(s3Client, atLeastOnce()).putObject(any(PutObjectRequest.class), any(RequestBody.class));
                return self();
        }

        public ThenWoodleViewMvcOutcome events_table_should_not_contain_participant(String participantName)
                        throws Exception {
                log.info("Verifying that participant {} is not present in the events table", participantName);
                Document doc = getDocumentFromResult();
                Elements participantRows = doc.select("table[data-test='events-table'] tbody tr");
                for (Element row : participantRows) {
                        Elements nameCells = row.select("td[data-test-participant='" + participantName + "']");
                        assertThat(nameCells.size()).isEqualTo(0);
                }
                return self();
        }

        public ThenWoodleViewMvcOutcome events_table_should_have_delete_buttons_for_participants() throws Exception {
                log.info("Verifying that delete buttons are present for each participant row");
                Document doc = getDocumentFromResult();
                Elements participantRows = doc.select("table[data-test='events-table'] tbody tr");

                for (Element row : participantRows) {
                        // Skip the new empty participant row (it shouldn't have a delete button)
                        Elements nameInputs = row.select("input[data-test='participant-name']");
                        if (nameInputs.size() > 0) {
                                continue; // This is the new participant input row
                        }

                        // Check for delete button in the last column
                        Elements deleteButtons = row
                                        .select("td:last-child button[data-test='delete-participant-button']");
                        assertThat(deleteButtons.size()).isEqualTo(1);

                        // Check that the delete button contains a trash can image
                        Element deleteButton = deleteButtons.first();
                        Elements trashImages = deleteButton.select("img[src='/trashcan.svg'][alt='Delete']");
                        assertThat(trashImages.size()).isEqualTo(1);
                }
                return self();
        }
}